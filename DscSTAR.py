...
def training(dataset, nn_paras, plotpath):
   ...
   print(batch_labels)
   data_res, batch_labels = ros.fit_resample(data, batch_labels)
...
def training_epoch(epoch, model, data_loader, nn_paras, classifier):
    ...
    if epoch % log_interval == 0:
        print('Avg_loss {:.12f}\t Avg_reconstruct_loss {:.12f}\t Avg_MSE_loss {:.12f}\t Avg_orthogonal_loss {:.12f}'.format(
        avg_total_loss, avg_reco_loss, avg_mse_loss, avg_ortho_loss))
    return avg_total_loss, avg_reco_loss, avg_mse_loss, avg_ortho_loss



def save_decoder_output(model, dataset, muX, file_path1, file_path2, plotpath, f='csv'):
    model.eval()
    gene_exp = dataset['gene_exp'].transpose()
    ...
    if f == 'csv':
        ...
        adata1 = ad.AnnData(df1)
        adata1.write(file_path1)
        adata2 = ad.AnnData(df2)
        adata2.write(file_path2)

    print(f"The shape of the output for output-1 is: {df1.shape}") 
    print(f"The shape of the output for output-2 is: {df2.shape}")
